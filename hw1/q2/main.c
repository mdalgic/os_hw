...
int write_bytes(const char* filename, const int byte_amount)
{
    ...
    fseek(file, byte_amount * -1, SEEK_END);
...
int write_lines(const char* filename, const int line_amount) 
{
    ...
    while (pos > 0 && lines_read < line_amount) {
        // Move the position indicator back one character
        fseek(file, --pos, SEEK_SET);

        // Read the character at the current position
        int c = fgetc(file);

        // If the character is a newline, we have read a line
        if (c == '\n') {
            lines_read++;
        }
    }

    // Move the position indicator back one character to skip the last newline
    fseek(file, ++pos, SEEK_SET);

    // Print the last N lines of the file to the terminal
    char line[1024];
...
{
    fprintf(stdout, "[HW1 Q2: kind of tail program]\n");
